...
import { fetchFacultyTypes, createFacultyType, updateFacultyType } from "@/api/academic";
import usePermission from "@/hooks/usePermission";

const INITIAL_FORM = { name: "", code: "", is_active: true };

export default function FacultyTypeMaster() {
  ...
  const drawerTitle = useMemo(() => (selectedRow ? "Edit Faculty Type" : "Create Faculty Type"), [selectedRow]);

  async function loadRows() {
    try {
      setLoading(true);
      const response = await fetchFacultyTypes();
      setRows(Array.isArray(response?.data) ? response.data : []);
      setError("");
    } catch (apiError) {
      setError(apiError?.response?.data?.message || "Failed to load faculty types.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (selectedRow) {
        await updateFacultyType(selectedRow._id, payload);
      } else {
        await createFacultyType(payload);
      }
      ...
      setError(apiError?.response?.data?.message || "Failed to save faculty type.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (!canView) {
    return (
      <main className="app-shell">
        <h3>Access denied</h3>
        <p>You do not have permission to view faculty types.</p>
      </main>
    );
  }
  ...
  return (
    <main className="app-shell">
      <PageHeader
        title="Faculty Type Master"
        subtitle="Configure different types of faculty (Permanent, Visiting, etc.)."
        actions={
          canCreate ? (
      ...
      {loading ? <p>Loading faculty types...</p> : <DataTable columns={columns} rows={tableRows} emptyMessage="No faculty types found." />}
      ...
      >
        <form className="ui-form-grid" onSubmit={handleSubmit}>
          <label>
            Name
            <input required name="name" value={form.name} onChange={handleFormChange} placeholder="e.g., Permanent" />
          </label>
          <label>
            Code
            <input required name="code" value={form.code} onChange={handleFormChange} placeholder="e.g., PERM" />
          </label>
